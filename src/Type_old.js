...
var Type = ( function()
{
    ...
    // and read from __scope__ to extract the private scope.
    ...
    function buildMembers( type, scope )
    {
        ...
        forEach( type.mixins, function( mixin )
        {
            ...
            tunnel.open( mixin );
            var inner = scope.self._pub.__scope__;
            tunnel.close();
    ...
    {
        if ( accessors.enumerable === undefined )
            accessors.enumerable = true;

        // IE8 apparently doesn't support this configuration option.
        if ( IE8 )
            delete accessors.enumerable;

        if ( accessors.configurable === undefined )
